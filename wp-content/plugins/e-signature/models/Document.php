...
class WP_E_Document extends WP_E_Model {
    ...
    public function getTitle($id) {
        ...
        $docTitle = $this->wpdb->get_var(
                $this->wpdb->prepare(
                        "SELECT document_title FROM " . $this->table . " WHERE document_id=%s LIMIT 1", $id
                )
        );
    ...
    public function document_document_page_exists($page_id) {
        ...
        $valid =["publish","private"];
        $postStatus= get_post_status ( $page_id );
        if(in_array($postStatus, $valid)){
            return true;
        }
        $pages =  get_pages(["include"=>[$page_id],"post_status"=>"publish,private"]);
