...
class WP_E_Email extends WP_E_Model {
    ...
    public function send($args = array()) {
        ...
        $email_template  = apply_filters('esig_email_template_file', $args['message_template'],$args['document']->user_id,$args['email_type']);

        $message = $args['message'];
        if ($args['message_template']) {
            $data = $this->get_default_data($args['template_data'], $args['document']->user_id);
            $message = WP_E_Sig()->view->renderPartial('', $data, false, false, $email_template);
        }
