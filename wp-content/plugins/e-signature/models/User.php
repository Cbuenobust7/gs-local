...
class WP_E_User extends WP_E_Model {
    ...
    public function get_all_esig_admins() {
        ...
        $user = $this->wpdb->get_results(
                $this->wpdb->prepare(
                        "SELECT * FROM " . $this->table . " WHERE wp_user_id != %d", 0
                )
        );
    ...
    public function getWpUsers(){
        ...
        if(class_exists('Esig_Roles')){
            $unlimitedRoles = Esig_Roles::get_unlimited_roles_option();
        }
        else {
            $unlimitedRoles = array();
        }
        if(!is_array($unlimitedRoles)){
             $unlimitedRoles = array();
        }

        $roles = array_merge($unlimitedRoles,[ 'administrator', 'editor','author','esig_sender' ]);
