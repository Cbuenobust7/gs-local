...
function esig_setup_tinymce_content() {
    
    if (!jQuery('input[name="esig_required_wpmember"]').is(":checked"))
        {
            return false;
        }
      
         let content = '';
        if (jQuery("#wp-document_content-wrap").hasClass("tmce-active")) {
            content = tinyMCE.activeEditor.getContent().replace(/<\/?[^>]+(>|$)/g, "");
        }
        else {
            content = jQuery('#document_content').val().replace(/<\/?[^>]+(>|$)/g, "");
        }
            var string_limit = 75;

            var short_description = content.substring(0, string_limit);

            jQuery("#esig_document_description").val(short_description);
...
(function ($) {
    ...
    $(function () {
        ...
        $('input[name="esig_required_wpmember"]').on('change', function ()
        {
            ...
            if ($('input[name="esig_required_wpmember"]').is(":checked"))
            {
            ...
            if (!$('input[name="esig_required_wpmember"]').is(":checked"))
        {
            ...
            var doc_content = $("#document_content").val().replace(/<\/?[^>]+(>|$)/g, "");
        ...
        $('#esig_document_description').on('input', function (e) {
            ...
            var ac_document_desc = $("#esig_document_description").val().replace(/<\/?[^>]+(>|$)/g, "");
...
// access control function 
    var access_role_validation = function () {
        ...
        if (jQuery('input[name="esig_required_wpmember"]').is(":checked"))
        {
             ...
             var user_selection = jQuery("#esig_access_roles_option").val();
