				...
				<tr style="border-collapse:collapse">
					<td style="font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.4em;border-collapse:collapse" align="center" bgcolor="#efefef">
						<table border="0" cellpadding="10" cellspacing="0" width="100%">
							<tbody>
								<tr style="border-collapse:collapse">
									<td style="font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.4em;border-collapse:collapse" align="left" width="100%">
										<div style="margin:0 0 20px 0"><?php $logo_alignment= apply_filters('esig-logo-alignment','',esigget('wpUserId', $data)); $logo_alignment = !empty( $logo_alignment) ?  $logo_alignment : 'style="text-align:left;"' ; ?>
											<div <?php echo $logo_alignment;  ?>> <?php echo $data['esig_logo']; ?></div>
												<p  <?php echo $logo_alignment;?>> <?php echo $data['esig_header_tagline']; ?><br></p> 
										</div> 
                                        <table width="100%">
											<tbody>
												<tr>
										...
										</table>                                                                                                                                                                <table width="100%" style="width: 100%;border-collapse: collapse; "><tbody><tr>
																																																		<td style="background-color:#ffffff;border:1px solid #ccc;padding:15px" bgcolor="#FFFFFF"><h1 style="font-size:18px;margin:0 0 10px 0;font-weight:bold"><?php _e('Document Name:', 'esig' );?> <?php echo $data['document_title']; ?></h1><?php _e('Document ID:', 'esig' );?> (<?php echo $data['document_checksum']; ?>)
                                        <p style="line-height:1.4em;font-size:14px;margin:10px 0px">
											<span style="color:#8c8c8c">
												<?php _e('From:','esig');?> <?php echo $data['user_full_name']; ?> (<a href="mailto:<?php echo $data['user_email'];?>" target="_blank"><?php echo $data['user_email']; ?></a>)
											</span> 
										</p>
										...
										<p style="line-height:1.4em;font-size:14px;font-family:sans-serif;margin:10px 0px">
													...
													<?php $custom_message = apply_filters('esig-invite-custom-message','',$document_checksum=$data['document_checksum']); echo $custom_message;?><br>
													<?php echo $data['user_full_name']; ?> <?php _e('sent you a document that needs to be signed.', 'esig' ); ?><br><br>
													<?php _e('Please add your signature to the document below.', 'esig' );?>  - <a href="mailto:<?php echo $data['user_email']; ?>?subject=RE%3A%20%7B<?php echo urlencode($data['document_title']); ?>%7D" style="color:#368bc6;text-decoration:none" target="_blank"><?php _e('Got a Question?', 'esig' );?><br></a>
										</p>
										<hr style="color:#cccccc;background-color:#cccccc;min-height:1px;border:none">
										<div style="margin:20px 0px 20px 0px"><?php $background_color_bg= apply_filters('esig-invite-button-background-color','',esigget('wpUserId', $data)); $background_color = !empty( $background_color_bg) ? $background_color_bg : '#0083c5' ;  ?>
											<table cellspacing="0" cellpadding="0">
												<tr>
													<td align="center" width="150" height="50" bgcolor="<?php echo $background_color ;  ?>" style="color: #ffffff; display: block;">
														<a href="<?php echo $data['invite_url']; ?>" style="font-size:14px; font-weight: bold; font-family:sans-serif; text-decoration: none; line-height:50px; width:100%; display:inline-block;"><span style="color: #ffffff;"><?php _e('REVIEW &amp; SIGN', 'esig' );?></span></a>
													</td>
												</tr>
											</table>
										</div>
							...
							<?php $footer_enable = apply_filters('esig-email-footer-text-enable','',esigget('wpUserId', $data)); if($footer_enable !='hide'){ ?> 
							<table width="100%">
								<tbody>
									<tr>
							...
							<tbody> 				  
								<tr style="border-collapse:collapse"> 					
									...
									<td style="width:15% !important;padding:0px 0px 0px 0px;vertical-align:top;font-family:'Helvetica Neue',Arial,Helvetica,sans-serif;font-size:12px;line-height:1.4em;border-collapse:collapse;color:#555" align="center"> 
										<a href="<?php echo  WP_E_Email::get_site_url(); ?>" target="_blank"><img src="<?php echo $data['assets_dir']; ?>/images/verified-email.jpg" style="width: 175px;margin-top: -8px;" class="emailClass" alt="WP E-Signature" border="0" height="69" width="200" ></a><br> 
									</td> 				  
								</tr> 				
							</tbody> 			  
						</table> 			
