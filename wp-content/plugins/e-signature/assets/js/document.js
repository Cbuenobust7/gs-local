...
//"use strict";
       ...
       // }
        ...
        jQuery.ajax({
            ...
            data: {
                ...
                esign_assign_signer_order: ($('#esig-signer-edit-wrapper #esign-assign-signer-order').is(':checked')) ? 1 : 0
            },
    ...
    $("#document_form").on("submit", function (e) {
        ...
        else if (isBlank(this['document_title'].value)) {
        ...
        //alert("");
        // Checking custom invite message validation
        if ($('input[name="esig_custom_message"]').is(":checked")) {
            
                    var invite_msg = $('#esig-custom-message-input textarea').val(); 
                   
                    if(invite_msg == ""){
                        $('#esig-custom-message-input textarea').css("border","1px solid red");
                         valid = false;
                    }
                    else {
                        $('#esig-custom-message-input textarea').css("border","0px solid");
                    }
	}
        
        // Checking custom invite message validation
        if ($('input[name="confirmation_custom_message"]').is(":checked")) {
            
                    var invite_msg = $('#confirmation_custom_message_text textarea').val(); 
                   
                    if(invite_msg == ""){
                        $('#confirmation_custom_message_text textarea').css("border","1px solid red");
                         valid = false;
                    }
                    else {
                        $('#confirmation_custom_message_text textarea').css("border","0px solid");
                    }
	}
        ...
        if (!valid) {
           // 
            ...
            if ($(".error").html() == undefined) {
                ...
                if (isBlank(this['document_title'].value))
                {
                    window.scrollTo(0, 0);
                    var alertmsg = '<div id="esig-doc-error" class="error"><p> Enter your documents name.</p></div>';
                }
