...
if (!class_exists('ESIG_USR_ADMIN')) :
    ...
    class ESIG_USR_ADMIN extends Esig_Roles {
            ...
            foreach ($docs as $doc) {
                ...
                if ($document_id) {
                    ...
                    if ($doc->user_id != $current_user_id) {
                        ...
                        if (!array_intersect($user_role, $esig_unlimited_roles_option) and !in_array($current_user_id, $esig_unlimited_users_option)) {
        ...
        public function esig_template_role($document_id) {
            ...
            $users_role_option = '';
            $wp_users =  WP_E_Sig()->user->getWpUsers(); 
...
';
        ...
        public function give_permissions($allcaps, $cap, $args) {
            ...
            if (in_array($wp_user_id, $esign_unlimited_users)) {
                ...
                $user_role = $this->get_current_user_role();
                if (array_intersect($user_role, $esign_unlimited_roles)) {
                    add_thickbox();
                    $allcaps['edit_posts'] = true;
        ...
        public function document_template_allow($document_id,$ownerId) {
            ...
            if (is_array($esign_unlimited_roles) && array_intersect($user_role,$esign_unlimited_roles)) {
                return true;
            } else {
                // if not found this role return false .
                return false;
        ...
        public function esig_document_search_filter() {
            ...
            if ($admin_user_id == $wp_user_id) {
                ...
                $wp_users = self::get_all_esig_admins(); //WP_E_Sig()->user->get_all_esig_admins();  // ;
                ...
                foreach ($wp_users as $key => $user) {
                    $users =get_user_by('id',$user->wp_user_id);
                    if(!$users) continue;
                    $selected = ($users->ID == $search_sender_id) ? "selected" : "";
                    if($admin_user_id==$user->wp_user_id) { continue; }
                    $all_sender .= '<option value="' . $users->ID . '" ' . $selected . '> ' . $users->display_name . '</option>';
                }
                $all_sender .='</select>';
            }
            return $all_sender;
        ...
        public function esign_unlimited_access_control() {
            ...
            if (is_array($esign_unlimited_users) && in_array($wp_user_id, $esign_unlimited_users)) {
                return "allow";
            } elseif (is_array($esign_unlimited_roles) && array_intersect($user_role, $esign_unlimited_roles)) {
                return "allow";
            } else {
                return "deny";
            }
        ...
        public function esign_unlimited_sender_role_view() {
            ...
            if (count($_POST) > 0) {
               ...
               // if (isset($_POST['esig_roles_option']) || isset($_POST['esig_roles_user_option'])) {
                    ...
                    $api->notice->set('alert e-sign-alert esig-updated', __('Well done ', 'esig') . '</strong> :  ' . __('Your E-Signature roles have been updated!', 'esig'));
                //}
            }
            ...
            // displaying users roles option 	
            $users_role_option = '';
            $wp_users = Esig_Roles::getWpUsers($esig_unlimited_roles_option);
            //getting users roles option from settings table 
