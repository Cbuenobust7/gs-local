...
if (!class_exists('ESIG_LOGO_BRANDING_Admin')) :
    ...
    class ESIG_LOGO_BRANDING_Admin extends esigBrandingSetting {
        ...
        public function __construct() {
            ...
            add_filter('esig_email_template_file', array($this, 'esig_email_template_file'), 10, 3);
            
        }
        
        public function esig_email_template_file($templatePath,$wpUserId,$templateType) {
            
            if(!$this->esig_plain_email_template($wpUserId)){
                return $templatePath;
            }
            
            if($templateType == "invitation"){
                return ESIGN_PLUGIN_PATH . ESIG_DS . 'views' . ESIG_DS . 'invitations' . ESIG_DS . 'invite_plain_template.php'; 
            }
            
            if($templateType == "signer-confirmation"){
                return ESIGN_PLUGIN_PATH . ESIG_DS . 'views' . ESIG_DS . 'notifications' . ESIG_DS . 'document_signed_plain_template.php';
            }
            
            if($templateType == "owner-confirmation"){
               return ESIGN_PLUGIN_PATH . ESIG_DS . 'views' . ESIG_DS . 'notifications' . ESIG_DS . 'admin' . ESIG_DS . 'document_signed_plain_template.php';
            }
            
            if($templateType == "view-notification"){
               return ESIGN_PLUGIN_PATH . ESIG_DS . 'add-ons' . ESIG_DS . 'esig-document-activity-notifications' . ESIG_DS . 'admin' . ESIG_DS . 'views' . ESIG_DS . 'notify_plain_template.php';
            }
            
            if($templateType == "reminders"){
               return Esig_Addons::get_business_pack_path() . ESIG_DS . 'esig-signing-reminders' . ESIG_DS . 'admin' . ESIG_DS . 'views' . ESIG_DS . 'invite_plain_template.php';
            }
        ...
        public function pdf_front_page($pdf_front_page, $doc_id) {
            ...
            <div class="cover_body" align="center">';
                ...
                $pdf_front_page .='<span class="company_title">' . stripslashes($api->setting->get("company_logo",$documents->user_id)) . '</span><br>';
        ...
        public function success_page_filter($success_msg, $args) {
            ...
            if (!$display) {
                ...
                elseif ($success_paragraph != false && $success_image == false) {
                    $success_msg = "<p class=\"success_title\" align=\"center\">{$success_paragraph}</h2> <p align='center' class='s_logo'><span class=\"esig-icon-success-check\"></span></p>";
                }
                else {
                    $success_msg = "<p class=\"success_title\" align=\"center\"><h2>{$successText} {$document_title}</h2> <p align='center' class='s_logo'><span class=\"esig-icon-success-check\"></span></p>";
                }
            } else {
        ...
        public function esign_upload_success_page_view($esig_branding_more_content) {
            ...
            if (count($_POST) > 0 && isset($_POST['mails-submit']) && $_POST['mails-submit'] == 'Save Settings') {
                ...
                $msg .= '<div class="alert e-sign-alert esig-updated"><div class="title"></div><p class="message">' . __('<strong>Well done </strong> :  Your E-Signature Success page settings have been updated!', 'esig') . '</p></div>';
            }
        ...
        public function esign_upload_logo_branding_view() {
            ...
            if (count($_POST) > 0 && isset($_POST['mails-submit']) && $_POST['mails-submit'] == 'Save Settings') {
                ...
                $this->save_esig_plain_email_template($wpUserId, ESIG_POST('esig_plain_email_template'));
                ...
                $msg .= '<div class="alert e-sign-alert esig-updated"><div class="title"></div><p class="message">' . __('<strong>Well done </strong> :  Your E-Signature E-mail templates settings have been updated!', 'esig') . '</p></div>';
            }
            ...
            $esig_plain_email_template = ($this->esig_plain_email_template($wpUserId)) ? "checked" : "";
            ...
            $template_data = array(
                ...
                'esig_plain_email_template' => $esig_plain_email_template,
            );
