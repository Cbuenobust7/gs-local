...
class esigBrandingSetting {
    ...
    public function documentHeadImage($wpUserId = false) {
     ...
     public function esig_plain_email_template($wpUserId = false) {
        $brandhingDisable = WP_E_Sig()->setting->get('esig_plain_email_template', $this->wpUserId($wpUserId));
        if ($brandhingDisable) {
            return $brandhingDisable;
        }
        return WP_E_Sig()->setting->get_generic('esig_plain_email_template' . $this->wpUserId($wpUserId));
    }
    ...
    public function save_esig_plain_email_template($wpUserId,$value){
         $oldData = WP_E_Sig()->setting->get_generic('esig_plain_email_template' . $wpUserId);
         if($oldData){
             WP_E_Sig()->setting->delete_generic('esig_plain_email_template' . $wpUserId);
         }
         WP_E_Sig()->setting->set('esig_plain_email_template', $value);
    }
