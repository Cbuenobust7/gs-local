...
if (!class_exists('ESIG_REMINDERS_Admin')) :
    ...
    class ESIG_REMINDERS_Admin extends ESIG_REMINDERS_SETTINGS {
        ...
        private function send_reminder($document_id, $signer_id) {
            ...
            $email_template  = apply_filters('esig_email_template_file', $invite_template,$document->user_id,'reminders');

            $invite_message = $api->view->renderPartial('', $template_data, false, '', $email_template);
        ...
        public function esig_reminders_instant_email() {
            ...
            for ($i = 0; $i < count($_POST['esig_reminder_email']); $i++) {
                ...
                $subject = __("Reminder: ","esig") . $document->document_title . __(' is awaiting your signature', 'esig');
        ...
        public function esig_reminders_settings() {
            ...
            echo '<div id="esig_signer_reminder_settings">
					...
					<p>' . __('Send the first reminder to the signer', 'esig') . ' <input id="reminder_for" type="number" name="esig_reminder_for" value="' . $esig_reminder_for . '" min="0" oninput="this.value = (isNaN(Math.abs(this.value)))?0:Math.abs(this.value)" style="max-width:75px"> ' . __('days after the initial signing request. ', 'esig') . '</p>
				<p>' . __('Send the second reminder to the signer', 'esig') . '  <input id="reminder_repeat" type="number" name="esig_reminder_repeat" value="' . $esig_reminder_repeat . '"  min="0" oninput="this.value = (isNaN(Math.abs(this.value)))?0:Math.abs(this.value)" style="max-width:75px"> ' . __('days after the initial signing request. ', 'esig') . '</p>
				<p>' . __('Send the last reminder to the signer', 'esig') . ' <input id="reminder_expire" type="number" name="esig_reminder_expire" value="' . $esig_reminder_expire . '" min="0" oninput="this.value = (isNaN(Math.abs(this.value)))?0:Math.abs(this.value)" style="max-width:75px"> ' . __('days after the initial signing request. ', 'esig') . '</p>
        ...
        public function document_add_data($more_contents) {
			...
			<a href="#" class="tooltip">
				...
				<p>' . __('Send the first reminder to the signer ', 'esig') . ' <input type="text" name="esig_reminder_for" id="esig_reminder_for" min="0" oninput="this.value = (isNaN(Math.abs(this.value)))?0:Math.abs(this.value)" value="' . $esig_reminder_for . '"> ' . __('days after the initial signing request.', 'esig') . '</p>
				<p>' . __('Send the second reminder to the signer ', 'esig') . '  <input type="text" name="esig_reminder_repeat" id="esig_reminder_repeat" min="0" oninput="this.value = (isNaN(Math.abs(this.value)))?0:Math.abs(this.value)" value="' . $esig_reminder_repeat . '"> ' . __('days after the initial signing request.', 'esig') . '</p>
				<p>' . __('Send the last reminder to the signer ', 'esig') . '   <input type="text" name="esig_reminder_expire" id="esig_reminder_expire" min="0" oninput="this.value = (isNaN(Math.abs(this.value)))?0:Math.abs(this.value)" value="' . $esig_reminder_expire . '"> ' . __('days after the initial signing request.', 'esig') . '</p>
