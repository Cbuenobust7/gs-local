...
class ESIG_SAD {
    ...
    public function display_document($atts) {
       ...
       // update_option("rupom", $_REQUEST);
       $wpPageType = esigget("context",$_REQUEST);
       $esigLocale = esigget("_locale",$_REQUEST);
                if($wpPageType=="edit" || $esigLocale=="user"){
                    return;
                }
        ...
        $html = '
			<p>
				<label for="email" class="esig-visuallyhidden">' . __('Your email address', 'esig') . '</label><input required type="email" class="form-control" placeholder="' . __('Your email address', 'esig') . '" id="esig-sad-email" name="esig-sad-email" value="' . $legal_email . '"/>
			</p>
        ';
        ...
        if (!isset($_POST['recipient_signature']) && empty($_POST['recipient_signature']) && !isset($_POST['esignature_in_text']) && empty($_POST['esignature_in_text'])) {
            ...
            $esign_text = esigpost('esignature_in_text');
            
            if (empty($esign) && empty($esign_text)) {
            ...
            $invitation = array(
                ...
                "sender_name" => esigget("first_name", $owner) . ' ' . esigget("last_name", $owner),
            ...
            $success_msg = "<p class=\"success_title\" align=\"center\">" . sprintf(__('Excellent work! You signed {%s}.', 'esig'), $doc->document_title) . "</h2> <p align='center' class='s_logo'><span class=\"esig-icon-success-check\"></span></p>";
