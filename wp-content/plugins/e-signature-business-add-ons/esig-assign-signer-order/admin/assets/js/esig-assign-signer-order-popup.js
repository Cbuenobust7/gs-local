(function ($) {
    ...
    $('input[name="esig_assign_approval_signer"]').on('change', function () {
        ...
        if ($('input[name="esig_assign_approval_signer"]').is(":checked")) {
    ...
    $("#add-approval-signer").on("click", function (e) {
        ...
        if ($('#esig-approval-signer-setting #esign_assign_approval_signer_order').is(":checked")) {
    ...
    $('input[name="esign_assign_approval_signer_order"]').on('change', function () {
        ...
        if ($('input[name="esign_assign_approval_signer_order"]').is(":checked")) {
        ...
        for (i = 0; i < fname.length; i++) {
            ...
            if (i == 0) {
              ...
              html +='<div id="signer_main" class="row topPadding bottomPadding no-move">'+
                     '<div class="col-sm-1 noPadding" style="width:6% !important;"></div>' + 
                     '<div class="col-sm-5 noPadding" style="width:39% !important;"><input class="form-control esig-input" type="text" name="approval_signer_fname[]" value="' + fname[i] + '" placeholder="Signer 1(from website)" readonly /> </div>'+
                  '<div  class="col-sm-5 noPadding leftPadding-5" style="width:39% !important;"><input type="text" class="form-control esig-input" name="approval_signer_emails[]" value="' + signer_emails[i] + '" placeholder="Signer 1(from website)" readonly /></div>'+
                  '<div  class="col-sm-1  text-left" style="width:14% !important;"></div></div>';   
                ...
                html +='<div id="signer_main" class="row">'+
                  '<div class="col-sm-1 noPadding" style="width:6% !important;"><span id="signer-sl" class="signer-sl">' + j + '.</span><span class="field_arrows"><span id="esig_signer_up"  class="up"> &nbsp; </span><span id="esig_signer_down"  class="down"> &nbsp; </span></span></div>' + 
                  '<div class="col-sm-5 noPadding" style="width:39% !important;"><input class="form-control esig-input" type="text" name="approval_signer_fname[]" value="' + fname[i] + '" placeholder="Signers Name" /> </div>'+
                  '<div  class="col-sm-5 noPadding leftPadding-5" style="width:39% !important;"><input type="text" class="form-control esig-input" name="approval_signer_emails[]" value="' + signer_emails[i] + '" placeholder="email@address.com" /></div>'+
                  '<div  class="col-sm-1  text-left" style="width:14% !important;"><span id="esig-del-signer" class="deleteIcon"></span></div></div>';   
            }
