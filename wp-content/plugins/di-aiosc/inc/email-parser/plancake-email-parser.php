...
class PlancakeEmailParser {
    ...
    public function  __construct($emailRawContent) {
        ...
        $this->has_html = (strpos($this->emailRawContent,'Content-Type: text/html') !== false);
    ...
    private function extractHeadersAndRawBody()
    {
        ...
        foreach ($lines as $line)
        {
            ...
            if ($this->isLineStartingWithPrintableChar($line)) // start of new header
            {
                ...
                if($newHeader == 'from') {
                    //FROM
                    if(preg_match('/^From:\s+(.*?)\s*(?:\[mailto:|<)(.*?)(?:\]|>)*$/im',$line,$matches2)) {
                        $this->rawFields['from']['name'] = @$matches2[1];
                        $this->rawFields['from']['email'] = @$matches2[2];
                    }
                    if(empty($this->rawFields['from']['email'])) {
                        if(preg_match('/^From:\s+(.*?)$/im',$line,$matches2)) {
                            $this->rawFields['from']['name'] = '';
                            $this->rawFields['from']['email'] = @$matches2[1];
                        }
                    }
                    if(!empty($this->rawFields['from']['email']))
                        $this->rawFields['from']['email'] = str_replace(array("]","[","<",">"," "),"",trim($this->rawFields['from']['email']));

                }
                elseif($newHeader == 'to') {
                    //TO
                    if(preg_match('/^To:\s+(.*?)\s*(?:\[mailto:|<)(.*?)(?:\]|>)*$/im',$line,$matches2)) {
                        $this->rawFields['to']['name'] = @$matches2[1];
                        $this->rawFields['to']['email'] = @$matches2[2];
                    }
                    if(empty($this->rawFields['to']['email'])) {
                        if(preg_match('/^To:\s+(.*?)$/im',$line,$matches2)) {
                            $this->rawFields['to']['name'] = '';
                            $this->rawFields['to']['email'] = @$matches2[1];
                        }
                    }
                    if(!empty($this->rawFields['to']['email']))
                        $this->rawFields['to']['email'] = str_replace(array("]","[","<",">"," "),"",trim($this->rawFields['to']['email']));
                }
                else {
    ...
    public function getBody($returnType=self::PLAINTEXT)
    {
        ...
        if ($returnType == self::HTML && $this->has_html) {
        ...
        foreach ($this->rawBodyLines as $line) {
            if (!$detectedContentType) {
                ...
                foreach($boundaries as $boundary) {
                    if(strpos(trim($line),trim($boundary)) !== false) {
                        $boundary_line = true;
                        break;
                    }
                }
                if($boundary_line == false)
                    $body .= $line . "\n";
        ...
        if($charset != 'UTF-8') {
            // FORMAT=FLOWED, despite being popular in email-templates, it is not
            // supported by iconv
